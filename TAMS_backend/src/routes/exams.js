...
router.get("/", async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      data: await query.orderBy("exams.date", "desc"),
    });
  } catch (err) {
    next(err);
  }
  ...
  try {
    ...
    if (!batch_id || !subject || !exam_type || !date || !max_marks)
      return res
        ...
        .json({
          success: false,
          message: "batch_id, subject, exam_type, date, max_marks required",
        });
    ...
    res
      .status(201)
      .json({ success: true, data: await db("exams").where({ id }).first() });
  } catch (err) {
    next(err);
  }
});
...
router.post("/:id/results", async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(results))
      return res
        .status(400)
        .json({ success: false, message: "results array required" });
...
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    await db("exam_results").where({ exam_id: id }).delete();
    await db("exams").where({ id }).delete();
